...
export default {
  name: "GetCode",
  props: ["mobile", "apiUrl", "type", "phoneCode", "errTxt", "username",'btnText','codeType1'],
  mixins: [MessageBox],
  data() {
    return {
      codeType: false,
  ...
  methods: {
    ...
    fnGetParams() {
      ...
      if (vm.apiUrl == "IBM_UTILS_SENDMOBILECODE") {
        ...
        // 无参手机
      }else if (vm.apiUrl == "IBM_UTILS_SENDMAILCODE") {
        // 无参邮箱
      ...
      }else if(vm.apiUrl == 'IBM_UTILS_GETSECURITYCODE'){
          params = {
              codeType: vm.codeType1
          }
      }
      return params;
    },
